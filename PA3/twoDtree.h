...
private:
   ...
   * You may want a recursive helper function for this one.
   ...
   void renderTree(PNG & im, Node* node);

   // recursive helper function for prune
   void pruneHelper(Node* node, int tol);

   // prune helper to check if all nodes in subtree are within tol
   bool check_tol(Node* node, int tol, RGBAPixel avg);

   int pruneSizeHelper(Node* node, int tol);
